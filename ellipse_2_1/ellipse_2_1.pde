...
void setup(){
  ...
  secondsSinceFOffCourse=millis()/1000;
}
...
void draw(){
 ...
 for(int i=0;i<points1d.length;i+=2) {
   point(points1d[i]*scalar,points1d[i+1]*scalar);
   //delay(1000);
 }
 ...
 if((Math.sqrt(Math.pow(f.getDX()-f.getX(),2)+Math.pow(f.getDY()-f.getY(),2)) > range*2 || (f.getAngleChange() > 65 && Math.sqrt(Math.pow(f.getDX()-f.getX(),2)+Math.pow(f.getDY()-f.getY(),2)) > range*1.25))&&millis()/1000-secondsSinceFOffCourse>=2) {
   back2Path();
   secondsSinceFOffCourse=millis()/1000;
 }
   
 if(Math.sqrt(Math.pow(points1d[goingToPoint*2]*scalar-(f.getX()),2)+Math.pow(points1d[goingToPoint*2+1]*scalar-(f.getY()),2))<range) {
   goingToPoint++;
   if(goingToPoint*2==points1d.length)
     goingToPoint=0;
   
   f.setDestination(points1d[goingToPoint*2]*scalar,points1d[goingToPoint*2+1]*scalar);
 }
 
 if(millis()/10 - secondsSinceFUpdate >= 1) {
...
void createPoints() {
  ...
  points1d=new float[pointsPerQuarter*8];
  ...
  for(int i=0;i<quarterOfPoints;i++) {
    ...
    points1d[i*2] = (x_1+c)*cos(o)-y_1*sin(o);
    points1d[i*2+1] = y_1*cos(o)+(x_1+c)*sin(o);
    points1d[quarterOfPoints*4+i*2] =(-x_1+c)*cos(o)+y_1*sin(o);
    points1d[quarterOfPoints*4+i*2+1] =-y_1*cos(o)+(-x_1+c)*sin(o);
    ...
    points1d[quarterOfPoints*2+i*2] = (x_2+c)*cos(o)-y_2*sin(o);
    points1d[quarterOfPoints*2+i*2+1] = y_2*cos(o)+(x_2+c)*sin(o);
    points1d[quarterOfPoints*6+i*2] = (-x_2+c)*cos(o)+y_2*sin(o);
    points1d[quarterOfPoints*6+i*2+1] = -y_2*cos(o)+(-x_2+c)*sin(o);
  }
  if(goingToPoint*2>=points1d.length)
     goingToPoint=0;
  f.setDestination(points1d[goingToPoint*2]*scalar,points1d[goingToPoint*2+1]*scalar);
...
void back2Path(){
  ...
  float angleIs = 90;
      ...
      float ang = atan2(points1d[i+1]*scalar-f.getY(),points1d[i]*scalar-f.getX());
      if(ang<0)
        ang+=360;
      if(ang>=360)
        ang-=360;
     ...
     if( offset > f.getAngle()/radToDegrees - ang){
      ...
      float baseDist = (float)((angleChange/angleDivider)*(Math.pow(f.getX()-points1d[i]*scalar,2)+(Math.pow(f.getY()-points1d[i+1]*scalar,2))));
      ...
      if(baseDist/((goingToPoint+n)%(pointsPerQuarter*4)==i ? n:1) < dist) {
        ellipse(points1d[i]*scalar,points1d[i+1]*scalar,5,5);
        goingToPoint=i/2;
        System.out.println("NNNNNNNNNNNN");
